...
const StationMap = ({ groupedStations }) => {
  ...
  return (
    <div className="station-map">
        ...
        <div key={`line-${lineIndex}`} className="line-group">
          <div className="line-container">
            {lineGroup.stations.map((station, stationIndex) => {
              ...
              const monitorMatrix = Array.from({ length: sizeY }, (_, rowIndex) =>
                Array.from(
                  { length: sizeX },
                  (_, colIndex) => monitors[rowIndex * sizeX + colIndex] || "empty"
                )
              );
                    ...
                    {monitorMatrix.map((row, rowIndex) => (
                      <div key={`row-${rowIndex}`} className="station-row">
                        {row.map((cell, cellIndex) => (
                          <div
                            key={`cell-${cellIndex}`}
                            className={`station-cell ${cell === "empty" ? "empty" : "monitor"}`}
      ...
      {selectedMonitor && (
        <MonitorModal monitor={selectedMonitor} onClose={handleCloseMonitorModal} />
      )}
      {noMonitorCell && (
        <NoMonitorModal message={noMonitorCell} onClose={handleCloseNoMonitorModal} />
      )}
    </div>
  );
};
